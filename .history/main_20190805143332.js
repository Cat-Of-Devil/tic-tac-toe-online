...
const App = {
  ...
  onHashChange(){
    ...
    // let templateName = hash.replace(/(\/([^\/]+).*)/,"$2");
    // App.showScreen(templateName);

    if (hash == '/') {
      return App.showScreen('');
    } 
    ...
    if (/\/sessions\/(\d+)\/?$/.test(hash)) {
      let matches = hash.match(/\/sessions\/(\d+)\/?$/);
  ...
  showScreen(name){
    this.$templates.forEach(function(o, i){
      if (!o.matches('.js-'+name)) {

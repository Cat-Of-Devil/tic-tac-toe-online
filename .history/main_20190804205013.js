  ...
  function showPage(hash) {
    ...
    if (chunk[0] == 'battle') {
      ...
      if (gameId != '') {
        ...
        function renderGame(session, disableAll = false) {
          stateGame = Object.assign({}, session);
          container.innerHTML = '';
      
          let nextStepEl = document.querySelector('.js-battle .move span');
          nextStepEl.innerHTML = (session.step % 2 == 0) ? session.toolA_title : session.toolB_title;
          ...
          session.state.forEach((el, i) => {
            ...
            cssClass += (el !== null) || disableAll ? ' disabled' : '';

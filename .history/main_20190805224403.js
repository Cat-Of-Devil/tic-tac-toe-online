...
(function () {
  ...
  function onHashChange() {
    ...
    switch(screen) {
      case "sign-in": showSignInScreen(); break;
      case "sessions": showSessionsScreen(); break;
      case "battle": showBattleScreen(); break;
      case "rating": showRatingcreen(); break;
      default: alert('err')
    }
  }
  ...
  function showSignInScreen(){
    let parent = $('.js-sign-in');
    let usernameInp = $('input[type=text]', parent);
    let saveBtn = $('button', parent);
  ...
  function showSessionsScreen(){
    ...
    .then(resp => resp.json())
    .then((items)=>{
      let gameArea = $('.js-session-wrap', container);
      gameArea.innerHTML = '';
  ...
  function showBattleScreen(){
    let container = $('.js-battle .js-game');
    let hash = location.hash.slice(1);
    let gameId = hash.split('/')[1];
    let stateGame = {};

    if (!gameId) location.hash = 'sessions';
